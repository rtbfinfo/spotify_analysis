...
<div class="text-text text-center h-screen flex flex-col justify-center items-center gap-8 bg-cardBg z-0 sticky top-0">
    ...
    <h1 class="text-6xl font-extrabold tracking-tighter md:max-w-1/2" style="text-wrap: balance;">Comment <span class="text-primary">Spotify</span> a changé la façon de faire de la <span class="text-secondary">musique</span></h1>
    <div class="sound-wave flex gap-2 justify-center w-screen">
        {#each Array(150) as _, index (index)}
            <div class=" bar w-2 h-full bg-primary rounded-md" style="animation-duration:{Math.random() * (0.7 - 0.2) + 0.2}s"></div>
        {/each}
    </div>
    <h2 class="text-2xl tracking-tight">Sous-titre</h2>
    <h3 class="text-lg bg-cardBg p-2 rounded-lg tracking-tight text-primary max-w-fit bg-opacity-50 hover:bg-opacity-100 transition-all">Ambroise Carton et Héloïse Feldmann</h3>
